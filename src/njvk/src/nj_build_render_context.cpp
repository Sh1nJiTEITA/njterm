...
SBFrameData::Builder(ren::DeviceH device, vk::SharedImage swapchain_image,
    ...
    : device{device}, image{swapchain_image}, format{format}, attachments{att},
      extent{extent} {}
...
auto SBFrameData::BuildFramebuffer() -> vk::SharedFramebuffer { 
    std::vector< vk::ImageView > transformed_views;
    transformed_views.reserve(attachments.size() + 1);
    
    // Adding swapchain image view is needed
    for (auto& att : attachments)  {
        transformed_views.push_back(*att);
    }
    transformed_views.push_back(*data->view);
